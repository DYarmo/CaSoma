...
def normCOR(stack, obj):
    
    filename =  obj.DBpath
    #var_name_hdf5 = obj.DB['Animals'][obj.curAnimal][obj.curFOV][obj.dataFocus].name
    var_name_hdf5 = 'mov'
    #stack, TIME = getSubStack(obj)
    if 'mov' in obj.DB.keys():
        del obj.DB['mov']
    obj.DB.require_dataset('mov', shape = stack.shape, dtype=stack.dtype, data = stack)
    ...
    mc = MotionCorrect(filename, dview=dview, max_shifts=max_shifts,
                      ...
                      border_nan=border_nan, var_name_hdf5=var_name_hdf5)
    
    mc.motion_correct(save_movie=True)
    
    
    # load motion corrected movie
    m_cor = cm.load(mc.mmap_file)
